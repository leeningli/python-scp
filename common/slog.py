...
class slog(object):
    ...
    def __init__(self, filename, size=10, count=5, logid="qiueer", level= None, debug=False):
        try:
    ...
    def tolog(self, msg, level=None):
        try:
            ...
            fun = self._logger_method_map.get(level, self._logger.debug)
            fun(msg)
            lock.release()
        except Exception , expt:
            print traceback.format_exc()
        ...
        try:
            ...
            if order_keys:
                for key in order_keys:
                    ...
                    msgstr = u"%s%s%s%s\n" % (msgstr, key, fill, self._get_right_content(val))
    ...
    def _get_map_level(self,level="debug"):
        ...
        return lvlmap.get(level, logging.INFO)